    ...
    public function index()
{
    ...
    {
        // Lấy danh sách các order_id đã hoàn thành
        $completedOrderIds = DB::table('order_order_status')
            ->where('order_status_id', 5) // 5 = Đã hoàn thành
            ->where('is_current', 1)
            ->pluck('order_id');

        // Lấy đơn hàng đã hoàn thành của user hiện tại
        ...
        foreach ($orders as $order) {
            foreach ($order->items as $item) {
                $alreadyReviewed = Review::where('product_id', $item->product_id)
                    ->where('user_id', Auth::id())
...
public function store(Request $request)
{
    ...
    $review->is_active = null; // CHỜ DUYỆT - không tự động duyệt hoặc từ chối
    $review->reason = null; // Không có lý do từ chối
    $review->save();
