...
$(document).ready(function() {
    $(".confirm-text").off("click").on("click", function(e) {
        e.preventDefault();

        let button = $(this);
        let url = button.data('url'); // Lấy URL xóa
        let id = button.data('id'); // Lấy ID của phần tử
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Hiện thông báo thành công như hình bạn muốn
                        Swal.fire(
                            'Thành công!',
                            'Dữ liệu này của bạn đã được xóa.',
                            'success'
                        );
                    },
                    error: function() {
