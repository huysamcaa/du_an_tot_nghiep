        ...
        <div class="card">
            <div class="card-body">
                ...
                <form id="couponFilters" method="GET" action="{{ route('admin.coupon.index') }}" class="row g-2 mb-3">
                    {{-- Giữ các tham số khác khi submit (trừ page) --}}
                    @foreach (request()->except(['page']) as $k => $v)
                        @continue(in_array($k, ['search', 'perPage', 'is_active', 'discount_type', 'start_from', 'start_to', 'end_from', 'end_to']))
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="discount_type" class="form-control">
                            <option value="">-- Tất cả --</option>
                            <option value="percent" {{ request('discount_type') === 'percent' ? 'selected' : '' }}>Phần
                                trăm
                            </option>
                            <option value="fixed" {{ request('discount_type') === 'fixed' ? 'selected' : '' }}>Số tiền
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-12 d-flex" style="gap:8px;">
                        ...
                        @if (request()->hasAny(['search', 'is_active', 'discount_type', 'start_from', 'start_to', 'end_from', 'end_to']))
                ...
                <form method="GET" action="{{ route('admin.coupon.index') }}" class="d-flex align-items-center"
                    ...
                    <select name="perPage" id="perPage" class="form-control"
                        ...
                        @foreach ([10, 25, 50, 100] as $n)
                            <option value="{{ $n }}"
                                {{ request('perPage') == (string) $n ? 'selected' : '' }}>
                                {{ $n }}</option>
                        @endforeach
                    </select>
                </form>
