                    ...
                    <form action="{{ route('admin.products.update', $product->id) }}" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input type="checkbox" name="is_sale" value="1" class="form-check-input"
                        ...
                        <div class="mb-3">
                            ...
                            @if ($product->thumbnail)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $product->thumbnail) }}" width="200"
                                        class="img-thumbnail">
                                </div>
                            @endif
                        </div>
                                        ...
                                        @foreach ($product->variants as $i => $variant)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($variant->thumbnail)
                                                        <img src="{{ asset('storage/' . $variant->thumbnail) }}"
                                                            width="60"><br>
                                                    @endif
                                                    <input type="file" name="variants[{{ $i }}][thumbnail]"
                                                ...
                                                <td>
                                                    ...
                                                    @php
                                                        $colorValue =
                                                            $variant->attributeValues->where('attribute_id', 1)->first()
                                                                ->value ?? '';
                                                        $colorId =
                                                            $variant->attributeValues->where('attribute_id', 1)->first()
                                                                ->id ?? '';
                                                ...
                                                <td>
                                                    ...
                                                    @php
                                                        $sizeValue =
                                                            $variant->attributeValues->where('attribute_id', 2)->first()
                                                                ->value ?? '';
                                                        $sizeId =
                                                            $variant->attributeValues->where('attribute_id', 2)->first()
                                                                ->id ?? '';
