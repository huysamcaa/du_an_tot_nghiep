                                ...
                                @if ($brand->logo)
                                    <div class="mt-2">
                                        <img src="{{ asset('storage/' . $brand->logo) }}" width="100"
                                            alt="Logo thương hiệu" class="img-thumbnail">
                                    </div>
                                @endif
                            </div>
                            <div class="col-md-6">
                                <label for="is_active" class="font-weight-bold">Trạng thái <span
                                        class="text-danger">*</span></label>
                                <select name="is_active" id="is_active" class="form-control">
                                    <option value="1"
                                        {{ (string) old('is_active', (string) $brand->is_active) === '1' ? 'selected' : '' }}>
                                        Hiển thị</option>
                                    <option value="0"
                                        {{ (string) old('is_active', (string) $brand->is_active) === '0' ? 'selected' : '' }}>
                                        Ẩn</option>
                                </select>
                                @error('is_active')
                                    <small class="text-danger">{{ $message }}</small>
