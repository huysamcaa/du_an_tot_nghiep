    ...
    <tbody>
        @foreach($brands as $index => $brand)
            <tr>
                ...
                <a href="{{ route('admin.brands.edit', $brand->id) }}" class="btn btn-sm btn-warning icon-btn" title="Sửa">
                    <i class="fa fa-pencil"></i>
                </a>
                <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-sm btn-danger icon-btn" title="Xóa" onclick="return confirm('Bạn có chắc muốn xóa không?')">
