...
@foreach ($order->items as $item)
@php
$key = $order->id . '-' . ($item->product?->id ?? $item->id);
$review = $reviewDataMap[$key] ?? null;
$alreadyReviewed = $reviewedMap[$key] ?? false;
@endphp
            ...
            <form action="{{ route('client.reviews.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="d-flex align-items-start gap-3 mb-3">
                        <img src="{{ asset('storage/' . $item->product->thumbnail) }}" class="rounded" style="width: 80px; height: 80px; object-fit: cover;">
                        <div>
                    ...
                    <div class="mb-3 d-flex align-items-center gap-2 ms-3 mt-1">
                        ...
                        @if ($avatarPath)
                        <img src="{{ asset('storage/' . $avatarPath) }}" alt="Avatar" class="rounded-circle border" style="width: 40px; height: 40px; object-fit: cover;">
                        @else
                        <img src="{{ asset('assets/images/default-avatar.png') }}" alt="Avatar mặc định" class="rounded-circle border" style="width: 40px; height: 40px; object-fit: cover;">
                        @endif
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-1">
                            @for ($i = 1; $i <= 5; $i++) <input type="radio" name="rating" class="btn-check" id="star{{ $i }}-{{ $order->id }}-{{ $item->product->id }}" value="{{ $i }}" >
                                <label class="btn btn-outline-warning" for="star{{ $i }}-{{ $order->id }}-{{ $item->product->id }}">
...
<div class="modal fade" id="reviewModal-{{ $order->id }}-{{ $item->product->id }}" tabindex="-1" aria-labelledby="reviewModalLabel-{{ $order->id }}-{{ $item->product->id }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
        <div class="modal-content d-flex flex-column">
            <div class="modal-header">
                <h5 class="modal-title" id="reviewModalLabel-{{ $order->id }}-{{ $item->product->id }}">
                    Đánh giá sản phẩm
                </h5>
            ...
            <div class="modal-body overflow-auto" style="max-height: 70vh;">
                ...
                <div class="d-flex align-items-start gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <div class="text-warning">
                            @for ($i = 1; $i <= 5; $i++) <i class="fas fa-star {{ $i <= $review->rating ? '' : 'text-muted' }}"></i>
                                @endfor
                        </div>
                ...
                <p class="mb-3">Hình ảnh/ Video:
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <img src="{{ asset('storage/' . $media->file) }}" class="rounded border" style="width: 100px; height: 100px; object-fit: cover;">
