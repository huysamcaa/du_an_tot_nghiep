...
<head>
    ...
    <style>
        ...
        .address-item, .coupon-item {
            display: flex;
            padding: 15px 0;
            border-bottom: 1px solid #f2f2f2;
        }
        .address-item:last-child, .coupon-item:last-child {
            border-bottom: none;
        }
        .address-info, .coupon-info {
            flex-grow: 1;
        }
        .address-name, .coupon-code {
            font-weight: 600;
            margin-bottom: 5px;
            color: #2c3e50;
        }
        .address-details, .coupon-details {
            color: #666;
            font-size: 14px;
            margin-bottom: 5px;
        }
        .address-phone, .coupon-value {
            color: #666;
            font-size: 14px;
        }
        .select-address, .select-coupon {
            color: #3498db;
            font-size: 14px;
            cursor: pointer;
            display: flex;
        ...
        .address-actions, .coupon-actions {
            display: flex;
            align-items: center;
            gap: 15px;
        }
        .change-address-btn, .change-coupon-btn {
            color: #3498db;
            font-size: 14px;
            cursor: pointer;
            display: flex;
                            ...
                            <div class="checkout-card-body">
                                <div class="address-item">
                                    ...
                                    <div class="address-actions">
                                        <div class="change-address-btn" data-bs-toggle="modal" data-bs-target="#changeAddressModal">
                            ...
                            <div class="checkout-card-body">
                                <div class="coupon-item">
                                    <div class="coupon-info">
                                        ...
                                        <div class="coupon-value text-success fw-bold" id="couponValue" style="display: none;">
                                        </div>
                                    </div>
                                    <div class="coupon-actions">
                                        <div class="change-coupon-btn" data-bs-toggle="modal" data-bs-target="#changeCouponModal">
                                            <i class="fas fa-edit me-1"></i> Chọn Mã
                                        </div>
                                        <div class="select-coupon" data-bs-toggle="modal" data-bs-target="#addCouponModal">
                        ...
                        <div class="checkout-card">
                            ...
                            <div class="checkout-card-body">
                                <div class="payment-method">
                                    ...
                                    <img src="https://th.bing.com/th/id/OIP.pn3RUm1xk1HiAxWIgC6CIwHaHa?w=161&h=180&c=7&r=0&o=7&pid=1.7&rm=3" alt="VNPay" alt="VNPay" class="payment-icon">
                    ...
                    <div class="coupon-list">
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input coupon-radio" type="radio" name="selected_coupon" 
                                ...
                                {{ $coupon->pivot->used_at ? 'disabled' : '' }}>
                            <label class="form-check-label w-100" for="coupon{{ $coupon->id }}">
                                <div class="coupon-option {{ $coupon->pivot->used_at ? 'opacity-50' : '' }}">
                                    <div class="coupon-code">
                                        {{ $coupon->code }}
                                        @if($coupon->discount_type == 'percent')
                                            <span class="discount-badge">Giảm {{ $coupon->discount_value }}%</span>
                                        @else
                                            <span class="discount-badge">Giảm {{ number_format($coupon->discount_value) }}₫</span>
                                        @endif
                                        @if($coupon->pivot->used_at)
                                            <span class="badge bg-warning float-end">Đã sử dụng</span>
                                        @endif
                                    </div>
                                    <div class="coupon-desc">
                                        @if($coupon->discount_type == 'percent' && $coupon->restriction && $coupon->restriction->max_discount_value)
                                            Tối đa {{ number_format($coupon->restriction->max_discount_value) }}₫
                                        @endif
                                    </div>
                                    <div class="coupon-desc">
                                        {{ $coupon->description }}
